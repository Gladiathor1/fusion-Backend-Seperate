...
import OpenAIWhisperRequestHandler from './endpoints/service-proxies/openai/whisper';
import AutonomousHandler from './endpoints/service-proxies/langchain';
...
export default class ChatServer {
    ...
    async initialize() {
        ...
        this.app.get('/chatapi/myip', (req, res) => res.send(req.ip));
        ...
        // this.app.post("/chatapi/track", (req, res) => new Tracking(this, req, res))
        this.app.post("/chatapi/proxies/autonomous", (req, res) => new AutonomousHandler(this, req, res))
        ...
        if (config.services?.openai?.apiKey) {
            this.app.post('/chatapi/proxies/openai/v1/chat/completions', (req, res) => {

                new OpenAIProxyRequestHandler(this, req, res)
            });
            const upload = multer()
            this.app.post("/chatapi/proxies/openal/whisper",upload.any(), (req, res) => new OpenAIWhisperRequestHandler(this, req, res))
        }
        ...
        try {
            ...
            if (config.tls?.key && config.tls?.cert) {
                ...
                io.on("connection", (socket) => {
                    console.log("Socket Connected with someone")
                    new Tracking().handler(socket)
                })
